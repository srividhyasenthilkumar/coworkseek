...
export default function AllVirtualOfficePage() {
  ...
  const meetingRooms = allSpaces.filter((space) =>
    space.bookspace?.toLowerCase().includes("virtual"),
  );
  ...
  return (
    <section className="max-w-7xl mx-auto mt-30 px-4 py-6">
      <p className="text-sm text-gray-500 mb-4">Home / Virtual Office</p>

      <h1 className="text-2xl font-bold text-black">
        Virtual Office – All Locations
      </h1>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
        ...
        {filteredRooms.length === 0 && (
          <p className="text-gray-500">No Virtual Office available.</p>
        )}
      </div>
    </section>
  );
